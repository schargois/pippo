...
class Column_generator_LSTM(ProgColumnGenerator):
    ...
    def create_column(self, device):
        ...
        if self.num_dens_Layer == 0:
            ...
            self.num_dens_Layer == 1
        ):  # adding an extra dense layer between LSTM and output
            columns.append(
                ProgDenseBlock(
                    inSize=self.hidden_size,
                    outSize=32,
            ...
            self.num_dens_Layer == 2
        ):  # adding two extra dense layer between LSTM and output
            columns.append(
                ProgDenseBlock(
                    inSize=self.hidden_size,
                    outSize=64,
    ...
    def generateColumn(
        self, device, parent_cols
    ):  # generates column with its parents connections
        ...
        for i in range(1, len(new_column.blocks)):
            for j in range(len(parent_cols)):
                new_column.blocks[i].laterals.append(
